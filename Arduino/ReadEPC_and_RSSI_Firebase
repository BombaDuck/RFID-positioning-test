...
int readcountlist[10];
String readepclist[10];
...
void loop() 
{
  ...
  { 
    ...
    if(responseType == RESPONSE_IS_TAGFOUND)
    {
      ...
      int contain = contains(path);
      readcount = epclistarrangement(contain);

  
  //push RSSI
    //delay(500);
      if(Firebase.setInt(firebaseData, path + "antenna02/Data" + (readcount + 10000) + "/Rssi", rssi))
      {
...
int contains(String epcid)
{
  for(int i = 0 ; i<9 ; i++)
  {
    if(readepclist[i]== epcid)
    return i;
  }
  return -1;
}

int epclistarrangement(int contain)
{
  if(contain == -1)
  {
    for(int i = 0 ; i<9 ; i++)
    {
      if(readcountlist[i] == 0)
      {
         readepclist[i] = path;
         readcountlist[i] = 1;  
         return 1;
  ...
  {
    readcountlist[contain] = readcountlist[contain] + 1;
    return readcountlist[contain];
  }
}
